...
wsl.exe bash -c "cd $(wslpath -ua '%HERE%'); export PATH=\"~/.local/bin:$PATH\"; export ANSIBLE_LOG_PATH=./logs/ansible.$(date +%%Y%%m%%d_%%H%%M%%S).log; LANG=ja_JP.UTF-8 ansible-playbook -vvv -i hosts site.yml %*"
