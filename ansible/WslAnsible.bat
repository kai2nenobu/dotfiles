...
wsl.exe bash -c "cd $(wslpath -ua '%HERE%'); export ANSIBLE_LOG_PATH=./logs/ansible.$(date +%%Y%%m%%d_%%H%%M%%S).log; LANG=ja_JP.UTF-8 python3 $HOME/.poetry/bin/poetry run task apply %*"
